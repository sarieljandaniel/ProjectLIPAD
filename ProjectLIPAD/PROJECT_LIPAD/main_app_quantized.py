"""Project LiPAD — desktop app using quantized YOLOv8s + YOLOv8m ensemble."""
...
class LipadQuantizedApp(ctk.CTk):
    def __init__(
        ...
        super().__init__()
        self.weights_s_path = weights_s_path
        self.weights_m_path = weights_m_path
        self.title("Project LiPAD — Quantized Ensemble (YOLOv8s + YOLOv8m)")
    ...
    def _quantized_weights_dir(self) -> str:
        return os.path.join(self._repo_root(), "models", "quantized")

    def _default_quantized_weights(self) -> tuple[str, str]:
        qdir = self._quantized_weights_dir()
        return (
    ...
    def run_engine_from_ui(self) -> None:
        ...
        if inspection.lower() == "crack":
            weights_s = self.weights_s_path
            weights_m = self.weights_m_path
            if not os.path.exists(weights_s):
                self._set_status(f"Quantized YOLOv8s missing: {weights_s}")
                return
            if not os.path.exists(weights_m):
                self._set_status(f"Quantized YOLOv8m missing: {weights_m}")
                return
            cmd.extend(["--weights_s", weights_s, "--weights_m", weights_m])

        def _runner() -> None:
            self.after(0, lambda: self._set_status("Running quantized ensemble engine..."))
            try:
                with self.engine_lock:
                ...
                self.after(0, lambda: self._set_status("Ensemble engine complete. Open Analysis Overview."))
...
if __name__ == "__main__":
    ...
    qdir = os.path.join(root, "models", "quantized")
    weights_s = os.path.join(qdir, "ensemble_subset_s_fp16.onnx")
    weights_m = os.path.join(qdir, "ensemble_subset_m_fp16.onnx")
    app = LipadQuantizedApp(weights_s_path=weights_s, weights_m_path=weights_m)
    app.mainloop()
